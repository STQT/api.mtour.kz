...
SMSC_PASSWORD = settings.SMS_PASSWORD  # пароль
...
class SMSC(object):
    ...
    # 3 - hlr, 4 - bin, 5 - bin-hex, 6 - ping-sms, 7 - mms, 8 - mail, 9 - call, 10 - viber, 11 - soc)
    ...
    def get_sms_cost(self, phones, message, translit=0, format=0, sender=False, query=""):
        ...
        m = self._smsc_send_cmd(
            "send", "cost=1&phones=" + quote(phones) + "&mes=" + quote(message) +
                    ifs(sender is False, "", "&sender=" + quote(str(sender))) +
...
# Examples:
# smsc = SMSC()
# smsc.send_sms("+77787606608", "Текст на кириллице", sender="Mtour.kz")
